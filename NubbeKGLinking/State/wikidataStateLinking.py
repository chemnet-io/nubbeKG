...
SELECT ?state ?shortStateName
WHERE {
  ?state a nubbe:State .
  ?state rdfs:label ?shortStateName .
}
"""
...
def check_label_in_wikidata(label):
    state = label.strip()  # Entfernt unsichtbare Zeichen
    query = f"""
    SELECT ?state (SAMPLE(?stateShortName) AS ?shortStateName) WHERE {{
          ?state wdt:P31 wd:Q485258.
          ?state wdt:P1813 ?stateShortName.
          FILTER (STR(?stateShortName) = "{label}").
    }}
    GROUP BY ?state
    """
...
if nubbe_data:
    ...
    for item in nubbe_data['results']['bindings']:
        bioactivity_uri = item['state']['value']
        bioactivity_label = item['shortStateName']['value']
    ...
    for nubbe_uri, nubbe_label in nubbe_dict.items():
        adjusted_inchiKey = adjust_label(nubbe_label)
        print(f"Überprüfe Match {count} für URI: {nubbe_uri} mit InchiKey: {adjusted_inchiKey}")

        wikidata_matches = check_label_in_wikidata(adjusted_inchiKey)

        # Wenn Übereinstimmungen in Wikidata gefunden wurden
        if wikidata_matches:
            for match in wikidata_matches:
                wikidata_uri = match['state']['value']
                print(wikidata_uri)
                wikidata_label = match['shortStateName']['value']
                result.append({
                    'nubbe_uri': nubbe_uri,
    ...
    with open('NubbeKGLinking/State/merged_state_uris_wikidata.json', 'w') as f:
