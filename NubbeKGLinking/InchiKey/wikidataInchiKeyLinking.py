...
WHERE
  { ?identifier  a nubbe:UniqueIdentifiers. 
	?identifier nubbe:inchikey ?inchiKey
}
"""
...
def check_label_in_wikidata(label):
    inchi_key = label.strip()  # Entfernt unsichtbare Zeichen
    query = f"""
    SELECT ?identifier ?inchiKey WHERE {{
        ?identifier wdt:P235 "{inchi_key}".
        ?identifier wdt:P235 ?inchiKey .
    }}
    """
...
if nubbe_data:
    ...
    for item in nubbe_data['results']['bindings']:
        bioactivity_uri = item['identifier']['value']
        bioactivity_label = item['inchiKey']['value']
    ...
    for nubbe_uri, nubbe_label in nubbe_dict.items():
        ...
        if wikidata_matches:
            for match in wikidata_matches:
                wikidata_uri = match['identifier']['value']
                print(wikidata_uri)
                wikidata_label = match['inchiKey']['value']
                result.append({
                    'nubbe_uri': nubbe_uri,
    ...
    with open('NubbeKGLinking/InchiKey/merged_inchiKey_uris_wikidata.json', 'w') as f:
